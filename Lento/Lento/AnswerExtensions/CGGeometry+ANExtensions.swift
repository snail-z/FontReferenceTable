...
// MARK: - CGHorizontalEquantLayout

/// 水平分布
public struct CGHorizontalEquantLayout {
    
    /// 最大布局宽度，默认屏幕宽
    public let maxLayoutWidth: CGFloat = UIScreen.main.bounds.size.width
...
extension CGHorizontalEquantLayout {
    
    /// 固定间距获取合适的宽度
    public func equalWidthThatFits(_ fixedSpacing: CGFloat) -> CGFloat {
    ...
    /// 固定宽度获取合适的间距
    public func equalSpacingThatFits(_ fixedWidth: CGFloat) -> CGFloat {
        let value = maxLayoutWidth - leadSpacing - tailSpacing
        guard count > 1 else { return value }
