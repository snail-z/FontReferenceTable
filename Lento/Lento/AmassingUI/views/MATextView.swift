...
*  5. 输入框 contentSize 改变回调 - contentSizeChangedClosure(oldSize, newSize)
*  6. 输入框变化增加删除事件 - deleteBackward
*/
@objc open class MATextView: UITextView, UITextViewDelegate {
    ...
    /// 是否自适应内容高度，默认false (水平方向需要固定约束)
    @objc public var adjustsToFitContentHeightAutomatically = false

    /// 内容自适应限制的最大行数，默认0无限制
    @objc public var maxOfLinesForAutomaticallyLimit: NSInteger = 0 {
        didSet {
            guard oldValue != maxOfLinesForAutomaticallyLimit else {
                return
            }
            let oldSize = bounds.size
            invalidateIntrinsicContentSize()
            contentSizeChangedClosure?(oldSize, getIntrinsicContentSize())
        }
    }
    
    /// 输入框 contentSize 改变回调
    @objc public var contentSizeChangedClosure: ((_ oldSize: CGSize, _ newSize: CGSize) -> Void)?
    ...
    private func getIntrinsicContentSize() -> CGSize {
        ...
        if maxOfLinesForAutomaticallyLimit > 0 {
            size.height = min(heightForNumberOfLines(maxOfLinesForAutomaticallyLimit), size.height)
        }
        return size;
    }
    
    private var kContentSizeKey: String { return "contentSize" }
    
    private func bindNotifications() {
        NotificationCenter.default.addObserver(self, selector: #selector(textDidChange(_:)), name: UITextView.textDidChangeNotification, object: nil)
        addObserver(self, forKeyPath: kContentSizeKey, options: [.old, .new], context: nil)
    }
    
    private func unbindNotifications() {
        NotificationCenter.default.removeObserver(self, name: UITextView.textDidChangeNotification, object: nil)
        removeObserver(self, forKeyPath: kContentSizeKey)
    }
